...
class saidclass2(Node):
    ...
    def __init__(self):
        ...
        self.obj_pos = self.create_subscription(
            Path, "/local_plan", self.call_sub, 10)
        self.curve = 0.0
        self.x1 = 0.0
        self.y1 = 0.0
        self.x2 = 0.0
    ...
    def call_sub(self, msg):
        if len(msg.poses) <= 20:
            ...
            if self.curve == 10000:
                ...
                if msg.poses[10].pose.orientation.z < 0:
                    print("The robot is turning right with a curvature ")
                    print(self.curve)
                else:
                    print("The robot is turning left with a curvature ")
                    print(self.curve)
            else:
    ...
    def menger_curvature(self, point_1_x, point_1_y, point_2_x, point_2_y, point_3_x, point_3_y):
        if (math.sqrt(pow(point_1_x - point_2_x, 2)+pow(point_1_y - point_2_y, 2)) * math.sqrt(pow(point_2_x - point_3_x, 2)+pow(point_2_y - point_3_y, 2)) * math.sqrt(pow(point_3_x - point_1_x, 2)+pow(point_3_y - point_1_y, 2))) == 0:
            curvature = 10000
        else:
